...
2. **Read current mode** — Find the `yolo_mode:` field in BOARD.md. If missing, check whether invocation commands contain `--dangerously-skip-permissions` or `--dangerously-bypass-approvals-and-sandbox` to infer current mode.
...
- If running as root with a board user, the permission flags are on the inner command (after `sudo -u`), not the outer shell.
